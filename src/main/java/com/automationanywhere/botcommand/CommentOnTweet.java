...
@CommandPkg(
        ...
        name = "CommentOnTweet", label = "[[CommentOnTweet.label]]",
        node_label = "[[CommentOnTweet.node_label]]", description = "[[CommentOnTweet.description]]", icon = "pkg.svg",

        //Return type information. return_type ensures only the right kind of variable is provided on the UI.
        return_label = "[[CommentOnTweet.return_label]]", return_type = STRING, return_required = true, return_description = "[[CommentOnTweet.return.description]]")
public class CommentOnTweet {
    @Sessions
    ...
    public Value<String> action(
            ...
            //UI labels.
            @Pkg(label = "[[CommentOnTweet.originalTweet.label]]", description = "[[CommentOnTweet.originalTweet.description]]")
            //Ensure that a validation error is thrown when the value is null.
            @NotEmpty
            ...
            //UI labels.
            @Pkg(label = "[[CommentOnTweet.statusBody.label]]", description = "[[CommentOnTweet.statusBody.description]]")
            //Ensure that a validation error is thrown when the value is null.
            @NotEmpty
                    String statusBody,
            ...
            //UI labels.
            @Pkg(label = "[[CommentOnTweet.statusMedia.label]]", description = "[[CommentOnTweet.statusMedia.description]]")
        ...
        try {
            ...
            //Need original username to reply on status
            Status statusToReplyOn = twitter.showStatus(Long.parseLong(originalTweet));
            //Update status body with reply details to show on original user's timeline
            statusBody = "@"+statusToReplyOn.getUser().getScreenName()+" "+statusBody;
            //Create new Status object with statusBody
            StatusUpdate status = new StatusUpdate(statusBody);
            //Set in ReplyToID for a response to an original tweet
            status.setInReplyToStatusId(Long.parseLong(originalTweet));
            ...
            logger.info(LocalDateTime.now().toString() + " " + statusBody);

            //Send update to Twitter
            Status tweet = twitter.updateStatus(status);

            //Store result with Tweet ID
            result = "Successfully commented on tweet: id: " + tweet.getId();
        } catch (Exception e) {
